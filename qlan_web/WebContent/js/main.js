...
//    $urlRouterProvider.otherwise("/dashboard.html");  
    ...
    $stateProvider.state('index', {
            ...
            url: "/login",
            templateUrl: "qlan/login/login.html",
            ...
            url: "/changePassword",
            templateUrl: "qlan/login/changePass.html",
...
MetronicApp.run(["$rootScope", "settings", "$state", "gettextCatalog", "Idle","Restangular","Constant", function($rootScope, settings, $state, gettextCatalog, Idle,Restangular,Constant) {
   ...
   Restangular.setErrorInterceptor(function(response, deferred, responseHandler) {
        if(response.status === 401) {      
        	...
        	window.location="/qlan-web/#/login"
    ...
    if($rootScope.authenticatedUser==undefined || $rootScope.authenticatedUser ==null){
    	Restangular.one("usersService/keepalive").get().then(function(data){
...
MetronicApp.controller('AppController', ['$scope', '$rootScope', 'gettextCatalog', '$window','$translate','$state', function($scope, $rootScope, gettextCatalog, $window,$translate,$state) {
    ...
    $scope.$on('IdleTimeout', function() {
    	...
    	$window.location.href = '/qlan-web/#/login';
    });
...
MetronicApp.controller('TreeMenuController', ['$scope', function($scope) {
    $scope.$on('$includeContentLoaded', function() {
        Layout.initSidebar(); // init sidebar
    });
}]);
